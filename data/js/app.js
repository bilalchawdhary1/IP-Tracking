...
var body = document.getElementsByClassName('by')[0];
...
var el = document.querySelector('.nv');
el.onclick = function() {
...
if (typeof Index_Page !== 'undefined' && Index_Page == true) {
    ...
    if (document.getElementById('a')) {
        ...
        var ipv4Text = document.getElementsByClassName('ipv4')[0].textContent;

        ipv4.onclick = function() {
            navigator.clipboard.writeText(ipv4Text);
    ...
    // Display IPV6
    async function checkIPv6(DivId, url) {
        try {
            // Obtenir le contenu JSON de l'URL
            const response = await fetch(url);
            const data = await response.json();

            // Vérifier si 'ipv6' est égal à true
            if (data.ipv6 === true) {
                // Afficher la valeur de 'ip'
                document.getElementsByClassName(DivId)[0].textContent = data.ip;
            } else {
                // Message si 'ipv6' n'est pas égal à true
                document.getElementsByClassName(DivId)[0].textContent = "IPv6 is not enabled";
            }
        } catch (error) {
            // Gestion des erreurs
            document.getElementsByClassName(DivId)[0].textContent = "Error fetching data";
            console.error("Error:", error);
        }
    }

    // Appeler la fonction pour vérifier l'IPv6
    checkIPv6("ipv6", "https://get-ipv6.com");
...
if (typeof Google_Recaptcha !== 'undefined' && Google_Recaptcha == true) {
    ...
    sNew.async = true;
    sNew.src = "//www.google.com/recaptcha/api.js";
...
var sNew = document.createElement("script");
sNew.async = true;
sNew.src = "https://www.googletagmanager.com/gtag/js?id=G-35H8NXW5F4";
var s0 = document.getElementsByTagName('script')[0];
s0.parentNode.insertBefore(sNew, s0);
